(module conjure.client.lisp.swank
...
(defn connect [opts]
  (let [opts (or opts {})
        host (or opts.host (config.get-in [:client :lisp :swank :connection :default_host]))
        port (or opts.port (config.get-in [:client :lisp :swank :connection :default_port]))]
...
(defn on-filetype []
  (mapping.buf :n :LispDisconnect
               (config.get-in [:client :lisp :swank :mapping :disconnect])
               :conjure.client.lisp.swank :disconnect)
  (mapping.buf :n :LispConnect
               (config.get-in [:client :lisp :swank :mapping :connect])
               :conjure.client.lisp.swank :connect))
